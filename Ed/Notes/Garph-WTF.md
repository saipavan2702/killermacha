...
vector<int> dijkstra(int V, vector<vector<vector<int>>>& adj, int S) {
    ...
    // Insert the source with distance 0
    st.insert({0, S});
    dist[S] = 0;
...
vector<int> bellman_ford(int V, vector<vector<int>>& E, int S) {
    vector<int> dist(V, 100000000); // Initialize distances with a large value
    dist[S] = 0;
...
void Solve() {
    ...
    vector<vector<char>> G(n, vector<char>(p));
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < p; j++) {
            cin >> G[i][j];
        }
    ...
    while (!q.empty()) {
        ...
        while (sz--) {
            ...
            for (int i = 0; i < 4; i++) {
                ...
                if (ex >= 0 && ex < n && ey >= 0 && ey < p) {
                    if (G[ex][ey] == '#' && B > 0 && !dp[ex][ey][B - 1]) {
                        q.push({ex, {ey, B - 1}});
                        dp[ex][ey][B - 1] = 1;
                    }
