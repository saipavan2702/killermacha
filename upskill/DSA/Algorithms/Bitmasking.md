...
> [!summary]
> Bitmasks encode sets and boolean state into integer bits, enabling compact subset operations and dynamic-programming states.
...
| --- | --- | --- |
| Parity check | `x & 1` | `1` when odd |
| Clear lowest set bit | `x & (x - 1)` | Removes the rightmost `1` |
| Extract lowest set bit | `x & -x` | Isolates the rightmost `1` |
| Power-of-two check | `x && !(x & (x - 1))` | True for a positive power of two |
| Divide by two | `x >> 1` | Integer division by two |
| Multiply by two | `x << 1` | Multiplication by two |
| Uppercase to lowercase | `ch \| ' '` | Sets the ASCII case bit |
| Lowercase to uppercase | `ch & '_'` | Clears the ASCII case bit |

## Manipulating the k-th Bit

Use 1-based indexing for `k` in these examples.

> [!tip] Logic
> To target a specific bit, use a **mask**: `(1 << (k - 1))`.

* **Check if the k-th bit is set:** `(n & (1 << (k - 1))) != 0`
* **Turn on the k-th bit:** `n | (1 << (k - 1))`
* **Turn off the k-th bit:** `n & ~(1 << (k - 1))`
* **Toggle the k-th bit:** `n ^ (1 << (k - 1))`

## Clearing Bit Ranges

* **Clear LSB through the i-th bit:**
```cpp
mask = ~((1 << (i + 1)) - 1);
x &= mask;
...
* **Clear MSB through the i-th bit:**
```cpp
mask = (1 << i) - 1;
x &= mask;
...
## Why Each Element Appears in 2^(n-1) Subsets
For set {a, b, c} (n=3):
