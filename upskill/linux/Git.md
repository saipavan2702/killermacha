...
> [!info]
> `git clean -nfd`- dry run and shows the untracked files and directories
> `git diff HEAD origin/master --name-only` shows diff between remote and local with just file names
> `git log HEAD..origin/master --oneline` shows commits not in the current branch but in remote

>[!tip]
>### Prefer a Linear History?
>
>`git rebase origin/master` 
>Rebase replays your local commits on top of the remote's latest, giving you a clean linear history with no merge commits. 
>Good for personal feature branches before raising a PR. 
>On shared branches, stick to merge to avoid rewriting history others are working against.
